...
let myProd = document.querySelector('.products');
let myButtons = document.querySelectorAll('button');
myButtons.forEach(function (currentButton) {
...
function showCategory(category) {
    products[category].forEach(function (currentProd) {
        getProductMarkup(currentProd)
    })
};
...
function getProductMarkup(product) {
    myProd.insertAdjacentHTML('beforeEnd', `<div class = "product"><div>${product.name}</div><img src="${product.imageUrl}" alt=""><div>${product.price}</div><a href="https://example.com/producs/${product.id}">Подробнее</a><div>`);
}
