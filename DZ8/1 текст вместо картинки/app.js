...
function handleClick(clickedButtonEvent) {
    ...
    let buttonText = card.button.innerHTML;
...
function showMoreText(card) {
    ...
    let descrText = 'Вот такой вот крутой товар';

    let insertEl = document.createElement('div');
    insertEl.className = 'desc';
    insertEl.innerHTML = descrText;

    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1

    card.wrap.insertBefore(insertEl, card.productName.nextSibling);

    //card.wrap.insertBefore(`<div class = 'desc'>${descrText}</div>`, wrap.querySelector('img'));
